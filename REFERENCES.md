...
- **[2]** NVIDIA InfiniBand switch port-to-port latency — QM8790 product brief: **"sub-130ns port-to-port latency"** (the brief's own wording; an earlier draft of this file said "sub-90 ns", which the brief does not state); Quantum-X800 (<100 ns). Note: the QM9700 datasheet does not publish a latency figure; ~100 ns is the class figure for adjacent generations. https://network.nvidia.com/files/doc-2020/pb-qm8790.pdf
...
- **[7]** Polese et al., "Understanding O-RAN," arXiv:2202.01032 (IEEE Comms Surveys & Tutorials): three control loops — non-RT rApps (>1 s), near-RT RIC xApps (10 ms–1 s), real-time (<10 ms, in the O-DU/O-RU, not in a RIC). The survey notes these sub-10 ms loops "are not part of the current O-RAN architecture" and are referenced in specifications as for further study — i.e. the tier is named but unstandardized. (Paraphrase, not a quotation of a phrase the paper does not contain.)

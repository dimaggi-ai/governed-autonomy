...
**Implemented scope:** the [chaos-fidelity standard](https://github.com/dimaggi-ai/ai-cluster-chaos-fidelity) validates experiment specifications, not hardware outcomes; [reliability economics](https://github.com/dimaggi-ai/reliability-economics) models recovery tradeoffs. This repository provides architecture, a sourced latency exhibit, and a promotion gate—not an operational controller. L2+ promotion now requires authenticated runner receipts bound to an implementation digest. See [the evidence trust boundary](gate/EVIDENCE.md). Declaration-only examples and structural tests do not prove execution or earn promotion.
